...
export const customers = pgTable("customers", {
  ...
  customPricingRules: json("custom_pricing_rules").$type<{
...
export const customPricingRulesSchema = z.object({
  discountPercentage: z.number().min(0).max(100).optional(),
  fixedDiscount: z.number().min(0).optional(),
  specialRules: z.record(z.string(), z.union([z.string(), z.number(), z.boolean()])).optional(),
}).default({});
...
export const insertCustomerSchema = createInsertSchema(customers, {
  ...
  phone: z.string()
    .regex(/^(\+49|0049|0)?[1-9][0-9\s\-\(\)\.]{6,20}$/, "Invalid phone number format")
    .optional(),
...
export const insertCustomerContactSchema = createInsertSchema(customerContacts, {
  ...
  phone: z.string()
    .regex(/^(\+49|0049|0)?[1-9][0-9\s\-\(\)\.]{6,20}$/, "Invalid phone number format")
    .optional(),
  role: z.enum(["primary", "technical", "billing", "decision_maker", "other"]),
  department: z.string().max(100, "Department name too long").optional(),
});

export const selectCustomerContactSchema = createSelectSchema(customerContacts);
export const updateCustomerContactSchema = insertCustomerContactSchema.partial().omit({ 
  id: true, 
  createdAt: true 
});
...
export type ContactRole = "primary" | "technical" | "billing" | "decision_maker" | "other";